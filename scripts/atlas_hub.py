    ...
    for it in items:
        ...
        cards.append(
            ...
            + (f' data-shot style="--sc:{e(it["color"])};--shot:url({e(it["shot"])})'
               ...
               if it.get("shot")
               # ⛔ A DIFFERENT ATTRIBUTE AND A DIFFERENT PROPERTY. --shot means "a
               # school photographed this place" to every gate that reads this band; a
               # licensed picture of the craft answering to that name would make all of
               # them lie. Mirrors cardShell() in the template — change one, change both.
               else (f' data-illus style="--sc:{e(it["color"])};--illus:url({e(it["illus"])})'
                     + (f';--illusfocal:{e(it["illusFocal"])}' if it.get("illusFocal") else "")
                     + '"'
                     if it.get("illus") else f' style="--sc:{e(it["color"])}"'))
