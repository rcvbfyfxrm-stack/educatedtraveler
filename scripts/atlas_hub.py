...
.circle{text-align:center;border:1px solid var(--line);border-radius:18px;padding:clamp(30px,5vw,52px) 24px;margin:46px 0 18px;background:rgba(243,237,226,.02)}
.circle h2{font-family:'Fraunces',Georgia,serif;font-weight:300;font-size:clamp(24px,3.6vw,36px);margin:12px 0 12px}.circle p{color:var(--muted);max-width:52ch;margin:0 auto 22px;font-size:15px}
...
function cardInner(c){
  ...
  if(!c.open){
    return '<div class="shutflag">Not open yet</div>'+head
      ...
      +place
      +'<div class="askline">Ask me to open this one →</div>';
  }
  var st=star(c);
  return '<div class="openflag">Open</div>'
    ...
    +place
    +'<div class="cfoot">'+esc(levelCue(c.level))+(c.nDest>1?' · '+c.nDest+' places':'')+'</div>';
}
function card(c,cls){
  var col=WORLDS[worldOf(c)].color;
  return '<a class="'+cls+(c.open?'':' shut')+'" style="--sc:'+col+'" href="/atlas/'+esc(c.id)+'">'+cardInner(c)+'</a>';
}
...
function renderRails(){
  ...
  ORDER.forEach(function(w){var m=WORLDS[w],list=byWorld[w];if(!list.length)return;
    ...
    list.forEach(function(c){var a=document.createElement("a");a.className="dcard"+(c.open?"":" shut");a.style.setProperty("--sc",m.color);a.href="/atlas/"+c.id;a.innerHTML=cardInner(c);rail.appendChild(a);});
    rw.appendChild(rail);
    var larrow,rarrow;
    ...
    sec.appendChild(rw);results.appendChild(sec);
...
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,300..600;1,9..144,300..500&family=Inter:wght@300;400;500;600&family=IBM+Plex+Mono:wght@400;500&display=swap" rel="stylesheet">
{jsonld}
{analytics}
<style>{CSS}</style>
</head>
...
<div class="wrap"><div class="circle reveal">
  <div class="eyebrow">The Circle</div><h2 class="serif">Found the one that pulls you?</h2>
  <p>Open its page and write me a letter — that is how a craft gets opened, and how I know where to look next. Skills last, tans fade.</p>
  <a class="btn" href="/circle">Join the Circle</a>
</div></div>
